...
<Box display='grid' gap='100px' margin='auto' width='70%' >
        {projects.map((el)=>(
            <Box display={{base:'grid',sm:'grid',md:'grid',lg:'grid'}} className='main' width={{base:'100%'}}  gridTemplateColumns={{md:'repeat(2,1fr)',lg:'repeat(2,1fr)',base:'repeat(1,1fr)',sm:'repeat(1,1fr)'}} data-aos="flip-left" data-aos-easing="ease-out-cubic"  >
                 <Box >
                    <Image borderColor='yellow' border="10px solid #8A3FFC" boxSize={{md:'320px',lg:'320px',base:'270px',sm:'370px'}} src={el.img}></Image>
                 </Box>
                 <Box  w={{md:'400px',lg:'400px',base:'270px',sm:'340px'}}  textAlign='center' justifyContent='center'>
                    <Text color='#8A3FFC' fontStyle='oblique' fontWeight='100px' fontSize='40px' pt='30px' >{el.name}</Text>
